...
from .environ import Environ, checkpoint, interruptible_get
...
supply_cancel_page = 1 if os.environ.get("PKU_AUTOELECTIVE_DATA_DIR") else config.supply_cancel_page
...
def _ignore_course(course, reason):
    with environ.state_lock:
        ...
        environ.pending_courses.discard(course)
        environ.blocked_by_pending.discard(course)


def _has_pending_mutex(course_index):
    with environ.state_lock:
        return any(goals[int(peer)] in environ.pending_courses
                   for (peer,) in np.argwhere(mutexes[course_index, :] == 1))
...
def _fetch_course_page(client, page, limiter=None):
    """Every real page request, including the page-one recovery, uses one budget."""
    def request_first():
        if limiter is not None:
            limiter.wait()
        return client.get_SupplyCancel(username)

    if page == 1:
        response = request_first()
    ...
    for attempt in range(3):
        if limiter is not None:
            limiter.wait()
        response = client.get_supplement(username, page=page)
        tables = get_tables(response._tree)
        try:
            return response, get_courses(tables[1]), get_courses_with_detail(tables[0])
        except IndexError:
...
def run_iaaa_loop():
    ...
    while True:
        ...
        if elective is None:
            elective = interruptible_get(reloginPool)
        ...
        try:
            ...
            if os.environ.get("PKU_AUTOELECTIVE_DATA_DIR") and sys.exc_info()[0] is not None:
                # Propagating fatal errors (and cooperative cancellation) must
                # stop the peer loop before a potentially long retry delay.
...
def run_elective_loop():
    ...
    page_router = None
    page_limiter = None
    course_pages = {course: supply_cancel_page for course in cs.values()}
    if os.environ.get("PKU_AUTOELECTIVE_DATA_DIR"):
        from .desktop.runtime.pagination import PagePoolRouter, PageRateLimiter
        course_pages = {course: config.course_pages[cid] for cid, course in cs.items()}
        page_router = PagePoolRouter(course_pages.values(), elective_client_pool_size, config.page_pool_size)
        page_limiter = PageRateLimiter(refresh_interval, refresh_random_deviation, environ.stop_event)
        environ.page_allocations = page_router.allocations
    ...
    while True:
        ...
        if page_router is not None:
            # Hold all slots through the cross-page decision phase. A lease is
            # never handed to IAAA or another page's cycle while its href is used.
            for _ in range(elective_client_pool_size):
                client = interruptible_get(electivePool)
                leased[client.id] = client
            routes = page_router.next_routes()
            elective = leased[routes[0][1]]
        elif elective is None:
            elective = interruptible_get(electivePool)
        ...
        try:
            ...
            selected = list(dict.fromkeys(slot for _, slot in routes)) if routes else [elective.id]
            for slot in selected:
                if routes:
                    elective = leased[slot]
                if not elective.has_logined:
                    raise _ElectiveNeedsLogin
                if elective.is_expired:
                    try:
                        elective.logout()
                    except (AccessLimitedError, CaughtCheatingError):
                        raise
                    except Exception as e:
                        cout.warning("Logout error")
                        cout.exception(e)
                    raise _ElectiveExpired

            # Gather all configured pages before touching course state or making
            # decisions. Failure of any page invalidates this entire cycle.
            page_plans = {}
            sources = {}
            observed_at = {}
            elected = []
            plans = []
            fetch_routes = routes or [(supply_cancel_page, elective.id)]
            for page, slot in fetch_routes:
                if routes:
                    elective = leased[slot]
                page_r, elected_here, plans_here = _fetch_course_page(elective, page, page_limiter)
                page_plans[page] = plans_here
                observed_at[page] = time.time()
                elected.extend(elected_here)
                plans.extend(plans_here)
                for course in plans_here:
                    if course_pages.get(course) == page:
                        sources[course.to_simplified()] = (elective, page_r, page)
            ...
            tasks = [] # [(ix, course)]
            with environ.state_lock:
                environ.course_details.update({c.to_simplified(): c.status for page, catalog in page_plans.items()
                                               for c in catalog if course_pages.get(c) == page})
                environ.page_last_seen.update(observed_at)
                environ.blocked_by_pending.clear()
            for ix, c in enumerate(goals):
                if c in ignored:
                    ...
                    with environ.state_lock:
                        pending_confirmation = c in environ.pending_courses
                    if os.environ.get("PKU_AUTOELECTIVE_DATA_DIR") and pending_confirmation:
                        # A timed-out submission may still be processed by the server.
                        # Keep refreshing authoritative results, but never replay it.
                        cout.info("Course submission is awaiting confirmation")
                        continue
                    if page_router is not None and _has_pending_mutex(ix):
                        with environ.state_lock:
                            environ.blocked_by_pending.add(c.to_simplified())
                        continue
                    for c0 in page_plans[course_pages[c]]: # c0 has detail
                        if c0 == c:
                            if c0.is_available():
                                delay = delays[ix]
            ...
            while len(tasks) > 0:
                ...
                if routes:
                    elective, page_r, source_page = sources[course.to_simplified()]
                ...
                if page_router is not None and _has_pending_mutex(ix):
                    with environ.state_lock:
                        environ.blocked_by_pending.add(course.to_simplified())
                    continue
                ...
                try:
                    ...
                    checkpoint()
                    with environ.state_lock:
                        environ.pending_courses.add(course.to_simplified())
            ...
            if page_router is not None:
                for client in leased.values():
                    electivePool.put_nowait(client)
                elective = None
            elif elective is not None: # change elective client
                electivePool.put_nowait(elective)
                elective = None
            ...
            if noWait:
                ...
                cout.info("")
            elif page_router is not None:
                cout.info("Global page polling interval controls the next request")
            else:
