...
def get_title(tree):
    ...
    if title is None: # 双学位 sso_login 后先到 主修/辅修 选择页，这个页面没有 title 标签
        return None
    return title.text
...
def get_errInfo(tree):
    ...
    for td in tds:
        ...
        if strong is not None:
            label = ''.join(strong.itertext()).strip().rstrip(':：').strip()
            if label in ('出错提示', '提示'):
                candidates.append(td)
    if len(candidates) != 1:
        raise ValueError('Unrecognized school error page structure')
    text = ''.join(candidates[0].itertext()).strip()
    return re.sub(r'^(?:出错提示|提示)\s*[:：]\s*', '', text).strip()
...
def get_tips(tree):
    ...
    td = cells[1] if len(cells) >= 2 else tips[0]
    return "".join(td.xpath('.//text()')).strip() or None
