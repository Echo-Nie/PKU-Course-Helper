...
class AutoElectiveConfig(BaseConfig, metaclass=Singleton):
    ...
    def __init__(self):
        if environ.config_parser is not None:
            self._config = environ.config_parser
        else:
    ...
    @property
    def page_pool_size(self):
        return self._config.getint("client", "page_pool_size", fallback=1)
    ...
    def course_pages(self):
        return OrderedDict((id_, self._config.getint(section, "page")
                            if self._config.has_option(section, "page") else self.supply_cancel_page)
    ...
    def courses(self):
        ...
        for id_, s in self.ns_sections('course'):
            ...
            # Page routing and extension keys belong to configuration, not to
            # the legacy Course identity used by rules and server matching.
            c = Course(name=d['name'], class_no=d['class'], school=d['school'])
            cs[id_] = c
            rid = rcs.get(c)
            if rid is not None:
