...
# Desktop workers never write alongside bundled resources or to the user profile.
_portable_data = os.environ.get("PKU_AUTOELECTIVE_DATA_DIR")
if _portable_data:
    CACHE_DIR = os.path.join(_portable_data, "cache")
    CAPTCHA_CACHE_DIR = os.path.join(CACHE_DIR, "captcha")
    LOG_DIR = os.path.join(_portable_data, "logs")
...
if getattr(sys, "frozen", False):
    USER_AGENTS_TXT_GZ = os.path.join(sys._MEIPASS, "user_agents.txt.gz")
if _portable_data:
    USER_AGENTS_USER_TXT = os.path.join(_portable_data, "user_agents.user.txt")
    DEFAULT_CONFIG_INI = os.path.join(_portable_data, "config.ini")
