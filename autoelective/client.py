...
from .environ import checkpoint
...
class BaseClient(object):
    ...
    def _request(self, method, url,
        ...
        try:
            ...
            # School hooks may raise before Requests consumes/releases the body.
            # Close the response even on cancellation; do not accumulate sockets.
            response = getattr(error, 'response', None)
            if response is not None:
                try:
                    response.close()
                except Exception:
                    pass  # Preserve the original, more useful failure.
            raise
